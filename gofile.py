...
class Downloader:
    def __init__(self, url_or_file: str, password: str | None = None, download_path: str | None = None) -> None:
        root_dir: str | None = download_path or getenv("GF_DOWNLOAD_DIR")
        ...
        if not path.exists(self._root_dir):
            try:
                makedirs(self._root_dir, exist_ok=True)
                _print(f"Created download directory: {self._root_dir}{NEW_LINE}")
            except OSError as e:
                die(f"Failed to create download directory '{self._root_dir}': {e}")
        elif not path.isdir(self._root_dir):
            die(f"Download path '{self._root_dir}' exists but is not a directory")
    ...
    def run(self) -> None:
        ...
        _print(f"Starting download to: {self._root_dir}{NEW_LINE}")
        _print(f"Please wait...{NEW_LINE}")
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="GoFile downloader with customizable download location",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python gofile-downloader.py https://gofile.io/d/contentid
  python gofile-downloader.py https://gofile.io/d/contentid password
  python gofile-downloader.py -pp /path/to/downloads https://gofile.io/d/contentid
  python gofile-downloader.py -pp /path/to/downloads https://gofile.io/d/contentid password
        """)
    
    parser.add_argument(
        '-pp', '--path',
        dest='download_path',
        help='Download location path (overrides GF_DOWNLOAD_DIR environment variable)'
    )
    
    parser.add_argument(
        'url_or_file',
        help='GoFile URL or file containing URLs to download'
    )
